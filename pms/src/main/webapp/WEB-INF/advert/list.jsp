			   ...
			   if(confirm(msg)==true){
				   $("input[type=checkbox]:checked").each(function(){
					...
					if(ids.length==0){
						...
						location.href="advertsdelete.do?idss="+ids;	
					}
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${adverts }" var="advert">
			...
			<td align="center">
			<a href="advertget.do?id=${advert.id }" onclick="if(!confirm('您确定修改吗？')) {return false;}" class="pn-opt">修改</a> | <a href="advertdelete.do?id=${advert.id }" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a>
			</td>
		</tr>
		</c:forEach> 		
