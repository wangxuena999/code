...
<body>
	...
	<div class="box-positon">
		...
		<form class="ropt">
			<!-- 新增完毕，页面跳转到栏目信息页面 -->
			<input type="submit" onclick="this.form.action='channellist.do';"
				...
				<tbody id="tab_1">
					<%-- <tr>
						<td width="20%" class="pn-flabel pn-flabel-h">
							...
							<span class="pn-frequired">${errmsg }</span>
						</td>
					</tr> --%>
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent">
							<input type="text" class="required" name="cname" id="cname"
							value="${channel.cname }" maxlength="10" onblur="chkcname()"/>
						...
						<td width="80%" class="pn-fcontent">
							<select name="pid">
								<c:forEach items="${channelnames }" var="channelname">
									<!-- 显示所属栏目 -->
									<c:if test="${channel.pid==channelname.id }">
										<option value="${channelname.id }" selected="selected">${channelname.channelpidname }</option>
									</c:if>
									<!-- 显示其他栏目 -->
									<c:if test="${channel.pid!=channelname.id }">
										<option value="${channelname.id }">${channelname.channelpidname}</option>
									</c:if>
